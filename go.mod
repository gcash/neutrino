module github.com/gcash/neutrino

go 1.23.4

require (
	github.com/davecgh/go-spew v1.1.1
	github.com/gcash/bchd v0.20.0-rc1
	github.com/gcash/bchlog v0.0.0-20180913005452-b4f036f92fa6
	github.com/gcash/bchutil v0.0.0-20241220024231-3d67040d3361
	github.com/gcash/bchwallet v0.10.1-0.20241220234253-0350bbbbc8e3
	github.com/gcash/bchwallet/walletdb v0.0.0-20241221001704-23316621c8ac
)

require (
	github.com/aead/siphash v1.0.1 // indirect
	github.com/btcsuite/go-socks v0.0.0-20170105172521-4720035b7bfd // indirect
	github.com/btcsuite/golangcrypto v0.0.0-20150304025918-53f62d9b43e8 // indirect
	github.com/btcsuite/websocket v0.0.0-20150119174127-31079b680792 // indirect
	github.com/dchest/siphash v1.2.3 // indirect
	github.com/kkdai/bstream v1.0.0 // indirect
	github.com/zquestz/grab v0.0.0-20190224022517-abcee96e61b1 // indirect
	go.etcd.io/bbolt v1.3.11 // indirect
	golang.org/x/crypto v0.31.0 // indirect
	golang.org/x/sys v0.28.0 // indirect
	golang.org/x/text v0.21.0 // indirect
	lukechampine.com/uint128 v1.3.0 // indirect
)
